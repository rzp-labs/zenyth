"""Test LLMInterface requires basic generation methods.

This test validates that the LLMInterface protocol requires the essential
text generation methods (generate and complete_chat), ensuring providers
...
def test_llm_interface_has_required_generation_methods() -> None:
    """Test LLMInterface requires basic generation methods."""
    provider = MinimalLLMProvider()
    assert hasattr(provider, "generate")
    assert hasattr(provider, "complete_chat")
    assert callable(provider.generate)
    assert callable(provider.complete_chat)
